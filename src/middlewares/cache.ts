// BASED ON https://github.com/honojs/hono/blob/main/src/middleware/cache/index.ts
// WITH MODIFICATIONS TO WORK BETTER WITH MOJISTOW
...
export function cache(options: {
  ...
  bypassCacheHeaders?: Record<string, string>;
}): MiddlewareHandler {
  ...
  return async function cache(c, next) {
    if (options.bypassCacheHeaders) {
      for (const [header, value] of Object.entries(options.bypassCacheHeaders)) {
        if (c.req.header(header) === value) {
          await next();
          return;
        }
      }
